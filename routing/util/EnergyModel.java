...
public class EnergyModel implements ModuleCommunicationListener {
    ...
    public void update(NetworkInterface iface, ModuleCommunicationBus comBus) {
        ...
        if (simTime > this.lastUpdate && iface.isTransferring()) {
            ...
            ArrayList<Connection> con = (ArrayList) (iface.getConnections());
            int i = 0;
            int total = 0;
            for (Connection c : con) {
                if (c.isTransferring()) {
                    Message m = c.getMessage();
                    if (m.getProperty("HelloMessage.protoXchg") != null) {
                        i++;
                    }
                    total++;
                }
            ...
            reduceEnergy(((double) (total-i) / (double) total) * delta * this.transmitEnergy);
        }
