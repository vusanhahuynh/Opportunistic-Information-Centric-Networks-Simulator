...
public class IntCacheRouter extends ActiveRouter {
    ...
    public IntCacheRouter(Settings s) {
        super(s);
     ...
     */
    public IntCacheRouter(IntCacheRouter r) {
        super(r);
        fstHopEncounters = new HashMap<DTNHost, EncounterInfo>();
        secHopEncounters = new HashMap<DTNHost, Map<DTNHost, EncounterInfo>>();
        encountersHistory = new HashMap<DTNHost, ArrayList<Double>>();
    ...
    public MessageRouter replicate() {
        return new IntCacheRouter(this);
    }
    ...
    public void changedConnection(Connection con) {
        ...
        if (con.isUp()) {
            ...
            if (con.isInitiator(getHost())) {
                ...
                IntCacheRouter otherRouter = (IntCacheRouter) otherHost.getRouter();
    ...
    protected boolean makeRoomForContent(Content newContent) {
        ...
        while (freeBuffer < newContent.getSize()) {
            ...
            for (Connection con : getConnections()) {
                ...
                IntCacheRouter othRouter = (IntCacheRouter) other.getRouter();
    ...
    protected Message getNextMessageToRemove(boolean excludeMsgBeingSent) {
        ...
        Collections.sort(validMessages,
                new IntCacheRouter.MaxPropComparator(this.calcThreshold()));
    ...
    private ArrayList<Tuple<Message, Connection>> socialTheorySort(List<Tuple<Message, Connection>> messages) {
        ...
        for (Tuple<Message, Connection> tmp : messages) {
            ...
            if (m.getTo() != null) {
                ...
                if (((IntCacheRouter) e.getRouter()).getSocialTieValue(dest) > this.getSocialTieValue(dest)) {
             ...
             forwardList.add(new Tuple<Message, Connection>(m, c));
                ...
                Connection c = tmp.getValue();
                DTNHost e = c.getOtherNode(getHost());
                if (((IntCacheRouter) e.getRouter()).getSocialCentrality() > this.getSocialCentrality()) {
                    //System.out.println(((IntCacheRouter) e.getRouter()).getSocialCentrality());
                    //forwardList.add(new Tuple<Message, Connection>(m, c));
                }
