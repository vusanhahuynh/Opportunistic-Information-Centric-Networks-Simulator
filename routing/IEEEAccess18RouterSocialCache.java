...
public class IEEEAccess18RouterSocialCache extends ActiveRouter {
    ...
    //////////////////////////////////////// Congestion Avoidance /////////////////////////////////////////
    protected Map<Integer, ArrayList<Double>> contentEncountersHistory;

    public IEEEAccess18RouterSocialCache(Settings s) {
        super(s);
        ...
        contentEncountersHistory = new HashMap<Integer, ArrayList<Double>>();
     ...
     */
    public IEEEAccess18RouterSocialCache(IEEEAccess18RouterSocialCache r) {
        super(r);
        fstHopEncounters = new HashMap<DTNHost, EncounterInfo>();
        secHopEncounters = new HashMap<DTNHost, Map<DTNHost, EncounterInfo>>();
        encountersHistory = new HashMap<DTNHost, ArrayList<Double>>();
    ...
    public MessageRouter replicate() {
        return new IEEEAccess18RouterSocialCache(this);
    }
    ...
    public void changedConnection(Connection con) {
        ...
        if (con.isUp()) {
            ...
            if (con.isInitiator(getHost())) {
                ...
                IEEEAccess18RouterSocialCache otherRouter = (IEEEAccess18RouterSocialCache) otherHost.getRouter();
    ...
    public Message messageTransferred(String id, DTNHost from) {
        ...
        if (m.getContent() != null && m.getContent().getContentType() == ContentType.Interest) {
            if (contentEncountersHistory.containsKey(m.getContent().getContentId())) {
                ArrayList<Double> tmp = contentEncountersHistory.get(m.getContent().getContentId());
                tmp.add(SimClock.getTime());
            } else {
                ArrayList<Double> tmp = new ArrayList<>();
                tmp.add(SimClock.getTime());
                contentEncountersHistory.put(m.getContent().getContentId(), tmp);
            }
        }
    ...
    private double getContentSocialTieValue(Content content) {
        if (!contentEncountersHistory.containsKey(content.getContentId())) {
            return 0.0;
        }
        double R = 0.0;
        ArrayList<Double> tmp = contentEncountersHistory.get(content.getContentId());
    ...
    protected Content getNextContentToRemove(Content newContent, boolean excludeMsgBeingSent) {
        ...
        for (Content c : contentStore) {
            ...
            if (oldest == null) {
                oldest = c;
            } else if (getContentSocialTieValue(oldest) > getContentSocialTieValue(c)) {
                // else if (getContentSocialTieValue(oldest) > getContentSocialTieValue(c)) {
                oldest = c;
            }
        }
        if (oldest != null && getContentSocialTieValue(newContent) > getContentSocialTieValue(oldest)) {
            return oldest;
        } else {
            return null;
        }
    ...
    @Override
    protected Message getNextMessageToRemove(boolean excludeMsgBeingSent) { // Replacement
        Collection<Message> messages = this.getMessageCollection();
        List<Message> validMessages = new ArrayList<Message>();

        int track = 0;
        int i = 0;
        double socialityValueBase = Double.MAX_VALUE;
        for (Message m : messages) {
            ...
            if (m.getTo() != null) {
                DTNHost to = m.getTo();
                if (this.getSocialTieValue(to) <= socialityValueBase) {
                    socialityValueBase = this.getSocialTieValue(to);
                    track = i;
                }
            } else {
                socialityValueBase = 0;
                track = i;
            }
            i++;
        }
        //System.out.printf("%d-%d ", track, validMessages.size() - 1);
        return validMessages.get(track);
    ...
    private Tuple<Message, Connection> tryOtherMessages() {
        ...
        for (Connection con : getConnections()) {
            ...
            IEEEAccess18RouterSocialCache othRouter = (IEEEAccess18RouterSocialCache) other.getRouter();
    ...
    private ArrayList<Tuple<Message, Connection>> socialTheorySort(List<Tuple<Message, Connection>> messages) {
        ...
        for (Tuple<Message, Connection> tmp : messages) {
            ...
            if (m.getTo() != null) {
                ...
                if (((IEEEAccess18RouterSocialCache) e.getRouter()).getSocialTieValue(dest) > this.getSocialTieValue(dest)) {
 ...
 /*else if (forwardList.size() < total - 1) {
             forwardList.add(new Tuple<Message, Connection>(m, c));
                ...
                Connection c = tmp.getValue();
                DTNHost e = c.getOtherNode(getHost());
                if (((IEEEAccess18RouterSocialCache) e.getRouter()).getSocialCentrality() > this.getSocialCentrality()) {
