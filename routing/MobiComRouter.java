...
/**
 * Implementation of MobiCom router as described in
 * <I>MobiCom: Routing for Vehicle-Based Disruption-Tolerant Networks</I> by
 * John Burgess et al.
 * @version 1.0
 *
 ...
 */
public class MobiComRouter extends ActiveRouter {
    /** Router's setting namespace ({@value})*/
	public static final String MAXPROP_NS = "MobiComRouter";
	/**
	 ...
	 */
	public MobiComRouter(Settings settings) {
		super(settings);
	 ...
	 */
	protected MobiComRouter(MobiComRouter r) {
		super(r);
		this.alpha = r.alpha;
	...
	public void changedConnection(Connection con) {
		...
		if (con.isUp()) { // new connection
			...
			if (con.isInitiator(getHost())) {
				...
				assert mRouter instanceof MobiComRouter : "MobiCom only works "+
				" with other routers of same type";
				MobiComRouter otherRouter = (MobiComRouter)mRouter;
	 ...
	 * at {@link ActiveRouter#update()}. MobiCom makes book keeping of the
	...
	/**
	 * Returns the next message that should be dropped, according to MobiCom's
	 * message ordering scheme (see MobiComTupleComparator).
	...
	protected Message getNextMessageToRemove(boolean excludeMsgBeingSent) {
		...
		Collections.sort(validMessages,
				new MobiComComparator(this.calcThreshold()));
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		for (Connection con : getConnections()) {
			...
			MobiComRouter othRouter = (MobiComRouter)other.getRouter();
		 ...
		 * defined in MobiComTupleComparator */
		Collections.sort(messages, new MobiComTupleComparator(calcThreshold()));
	...
	/**
	 * Message comparator for the MobiCom routing module.
	 ...
	 */
	private class MobiComComparator implements Comparator<Message> {
		 ...
		 */
		public MobiComComparator(int treshold) {
		 ...
		 */
		public MobiComComparator(int treshold, DTNHost from1, DTNHost from2) {
			this.threshold = treshold;
			this.from1 = from1;
			this.from2 = from2;
		}
		 ...
		 * {@link MobiComRouter#getCost(DTNHost, DTNHost)}) is first.
		 */
	...
	/**
	 * Message-Connection tuple comparator for the MobiCom routing
	 * module. Uses {@link MobiComComparator} on the messages of the tuples
	...
	public MessageRouter replicate() {
		MobiComRouter r = new MobiComRouter(this);
		return r;
	}
}