...
public class IEEEAccess18RouterHyMobi extends ActiveRouter {
    ...
    public IEEEAccess18RouterHyMobi(Settings s) {
        super(s);
     ...
     */
    public IEEEAccess18RouterHyMobi(IEEEAccess18RouterHyMobi r) {
        super(r);
        fstHopEncounters = new HashMap<DTNHost, EncounterInfo>();
        secHopEncounters = new HashMap<DTNHost, Map<DTNHost, EncounterInfo>>();
        encountersHistory = new HashMap<DTNHost, ArrayList<Double>>();
    ...
    public MessageRouter replicate() {
        return new IEEEAccess18RouterHyMobi(this);
    }
    ...
    public void changedConnection(Connection con) {
        ...
        if (con.isUp()) {
            ...
            if (con.isInitiator(getHost())) {
                ...
                IEEEAccess18RouterHyMobi otherRouter = (IEEEAccess18RouterHyMobi) otherHost.getRouter();
    ...
    public double getReputation(DTNHost specificHost) {
        ...
        for (Map.Entry<DTNHost, ArrayList<Double>> entry : reputationHistory.entrySet()) {
    ...
    protected boolean makeRoomForContent(Content newContent) {
        ...
        while (freeBuffer < newContent.getSize()) {
            ...
            for (Connection con : getConnections()) {
                ...
                IEEEAccess18RouterHyMobi othRouter = (IEEEAccess18RouterHyMobi) other.getRouter();
                if(othRouter.getFreeCacheSize() < c.getSize()) {
                    continue;
                }     
    ...
    @Override
    protected boolean makeRoomForMessage(int size) { // Reputation
    ...
    private ArrayList<Tuple<Message, Connection>> socialTheorySort(List<Tuple<Message, Connection>> messages) {
        ...
        for (Tuple<Message, Connection> tmp : messages) {
            ...
            if (m.getTo() != null) {
                ...
                if (((IEEEAccess18RouterHyMobi) e.getRouter()).getSocialTieValue(dest) > this.getSocialTieValue(dest)) {
 ...
 /*else if (forwardList.size() < total - 1) {
             forwardList.add(new Tuple<Message, Connection>(m, c));
                ...
                Connection c = tmp.getValue();
                DTNHost e = c.getOtherNode(getHost());
                if (((IEEEAccess18RouterHyMobi) e.getRouter()).getSocialCentrality() > this.getSocialCentrality()) {
