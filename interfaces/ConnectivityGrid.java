...
public class ConnectivityGrid extends ConnectivityOptimizer {
    ...
    /**
     * Cell based optimization cell size multiplier -setting id ({@value}). Used
     * in {@link World#OPTIMIZATION_SETTINGS_NS} name space. Single
     * ConnectivityCell's size is the biggest radio range times this. Larger
     * values save memory and decrease startup time but may result in slower
     * simulation. Default value is {@link #DEF_CON_CELL_SIZE_MULT}. Smallest
     * accepted value is 1.
     */
     ...
     *
     * @param key A hash value that separates different interfaces from each
     * other
    ...
    /**
     * Returns all interfaces that use the same technology and channel - interface as key? (VSHH comment)
     */
    ...
    public Collection<NetworkInterface> getNearInterfaces(
        ...
        /*Iterator iterator = DTNSim.allAvailableInterfaces.iterator();
        while (iterator.hasNext()) {
            NetworkInterface tmp = (NetworkInterface) iterator.next();
            niList.add(tmp);
